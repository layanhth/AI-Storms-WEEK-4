...
fail() { echo "GREEN CHECK: FAIL ($1)"; kubectl -n layan delete pod "$LOADGEN" --ignore-not-found >/dev/null 2>&1; exit 1; }

command -v kubectl >/dev/null || fail "kubectl -n layan not on PATH"
command -v helm >/dev/null || fail "helm not on PATH"

helm status "$RELEASE" -n layan >/dev/null 2>&1 || fail "no helm release '$RELEASE' (helm install $RELEASE ./serving-chart ...)"
kubectl -n layan get deployment "$DEPLOY" >/dev/null 2>&1 || fail "release exists but deployment $DEPLOY does not"

spec=$(kubectl -n layan get deployment "$DEPLOY" -o json)
...
maxunavail=$(kubectl -n layan get deployment "$DEPLOY" -o jsonpath='{.spec.strategy.rollingUpdate.maxUnavailable}')
[ "$maxunavail" = "0" ] || fail "chart strategy lost maxUnavailable: 0"
image=$(kubectl -n layan get deployment "$DEPLOY" -o jsonpath='{.spec.template.spec.containers[0].image}')
case "$image" in *"<your-user>"*) fail "release still runs the placeholder image" ;; esac

kubectl -n layan get hpa "$DEPLOY" >/dev/null 2>&1 || fail "no HPA named $DEPLOY (helm upgrade with --set hpa.enabled=true)"
minr=$(kubectl -n layan get hpa "$DEPLOY" -o jsonpath='{.spec.minReplicas}')
maxr=$(kubectl -n layan get hpa "$DEPLOY" -o jsonpath='{.spec.maxReplicas}')
[ "${maxr:-0}" -gt "${minr:-0}" ] || fail "HPA bounds $minr..$maxr leave no room to scale"

kubectl -n layan top pods >/dev/null 2>&1 || fail "kubectl -n layan top has no metrics; metrics-server is not serving (kind needs --kubelet-insecure-tls)"
...
model_id=$(kubectl -n layan get deploy "$DEPLOY" -o jsonpath='{.spec.template.spec.containers[0].env[?(@.name=="MODEL_ID")].value}' 2>/dev/null)
[ -n "$model_id" ] || model_id="Qwen/Qwen2.5-0.5B-Instruct"
api_key=$(kubectl -n layan get secret serving-keys -o jsonpath='{.data.api-key}' 2>/dev/null | base64 -d 2>/dev/null || true)

kubectl -n layan run "$LOADGEN" --image="$image" --restart=Never \
...
for _ in $(seq 1 36); do
  desired=$(kubectl -n layan get hpa "$DEPLOY" -o jsonpath='{.status.desiredReplicas}' 2>/dev/null)
  if [ "${desired:-0}" -gt "${minr:-1}" ]; then scaled="$desired"; break; fi
  sleep 5
done
kubectl -n layan delete pod "$LOADGEN" --ignore-not-found >/dev/null 2>&1
